...
<html lang="lt">
  <head>
    ...
    <link rel="stylesheet" href="game-3.css" />
...
<script>
  $(document).ready(() => {
    // $.get('http://***REMOVED***:4003/game3_server', data => {
    $.get('http://localhost:4003/game3_server', data => {
      ...
      $('#submit-answer').click(event => {
        ...
        if (checkLettersAndCompare(userAnswer, serverAnswer)) {
          ...
          // fetch('http://***REMOVED***:4002/points-server.js', {
          fetch('http://localhost:4004/points-server.js', {
            method: 'POST',
            headers: {
