...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="question-waiting.css">
</head>
<body>
...
document.querySelectorAll('.upvote').forEach(function(button) {
    button.addEventListener('click', function() {
        ...
        fetch('upvote.php?id=' + id + '&user_id=' + user_id)
...
document.querySelectorAll('.downvote').forEach(function(button) {
    button.addEventListener('click', function() {
        var id = this.dataset.id;
        var user_id = "<?php echo isset($_SESSION['user_id']) ? $_SESSION['user_id'] : ''; ?>";
        fetch('downvote.php?id=' + id + '&user_id=' + user_id)
            .then(function(response) {
                return response.text();
            })
            .then(function(text) {
            ...
            .catch(function(error) {
             ...
             <!-- perkelia klausimus i kita DB naudojant transfer-data.php -->
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
  $(function() {
    $("#btn-perkelti-klausimus").click(function() {
      $.ajax({
        url: "transfer-data.php",
        method: "post",
